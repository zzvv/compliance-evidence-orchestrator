...
// ctxErr normalizes a cancelled or expired context into the repository's
// cancellation sentinel so callers can match it with errors.Is. It is checked
// both before acquiring a lock and again after, so that a request cancelled
// while waiting for the lock is not resurrected into a successful result.
